...
func (gp *GoPdf) start(config Config, importer ...*pdfimporter.Importer) {
...
// init
func (gp *GoPdf) init(importer ...*pdfimporter.Importer) {
	gp.pdfObjs = []IObj{}
	gp.buf = bytes.Buffer{}
...
func importerOrDefault(importer ...*pdfimporter.Importer) *pdfimporter.Importer {
	if len(importer) != 0 {
		return importer[len(importer)-1]
	}
	return pdfimporter.NewImporter()
}
